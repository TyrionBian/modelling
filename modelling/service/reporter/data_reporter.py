...
def _gen_woe_report(df_train_origin, df_test_origin, label_col, report_dst, decimals=4, gen_report=False, **kwargs):
    df_train = df_train_origin.copy()
    df_test = df_test_origin.copy()
    # woe_report
    wr = feature_evaluation.WOEReport()
    d_tr = wr.fit_transform(df_train, label_col, **kwargs)
...
def _gen_warning_report(report_src, report_dst, gen_report=True):
    if os.path.exists(report_src) == False:
        ...
        df_result = df_result.sort_values(['feature_name','process_name'])
